...
def load_images_from_directory(directory_path: str, glob_pattern: str) -> list:
    ...
    loaded_images = []
    if not image_paths:
        print(f"Warning: No images found in '{directory_path}' matching '{glob_pattern}'")
...
def load_and_preprocess_image(image_item):  # Argument name 'image_item' matches mapspec
    ...
    """
    print(f"Processing image of shape: {image_item.shape if hasattr(image_item, 'shape') else 'N/A'}")
...
def segment_image(gray_image):  # Argument name 'gray_image' matches input from previous step
    """
...
def extract_feature(segmented_image):
    ...
    if not np.issubdtype(segmented_image.dtype, np.bool_):
        threshold = filters.threshold_otsu(segmented_image) if np.any(segmented_image) else 0.1
