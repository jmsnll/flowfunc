...
from flowfunc.console.application import COMMANDS as APP_DEFINED_COMMAND_NAMES

# Import your base Command classes to exclude them from discovery
from flowfunc.console.commands.command import Command as FlowFuncBaseCommand
from flowfunc.console.commands.command import WorkflowCommand as FlowFuncWorkflowCommand

# Determine the path to the main 'flowfunc' package directory and 'commands' directory
...
try:
    ...
    PIPEFLOW_PACKAGE_ROOT = Path(flowfunc.__file__).parent
    COMMANDS_MODULE_PATH = PIPEFLOW_PACKAGE_ROOT / "console" / "commands"
    # Add the project root to sys.path to ensure modules inside 'flowfunc' are discoverable
    # This is often needed if 'flowfunc' is not installed in editable mode during testing
    ...
    pytest.skip(
        f"Could not determine commands directory path: {e}. Ensure 'flowfunc' is discoverable.",
...
def discover_command_names_from_modules() -> set[str]:
    ...
    from classes inheriting from flowfuncBaseCommand.
    """
    ...
    for py_file in COMMANDS_MODULE_PATH.rglob("*.py"):  # Use rglob for recursive search
        # Construct the module import path relative to 'flowfunc.console.commands'
        ...
        module_import_path = f"flowfunc.console.commands.{sub_module_string}"
        ...
        try:
            ...
            for member_name, member_class in inspect.getmembers(
                ...
                if (
                    ...
                    if issubclass(
                        member_class, FlowFuncBaseCommand
                    ) and member_class not in [
                        FlowFuncBaseCommand,
                        FlowFuncWorkflowCommand,
                    ]:
